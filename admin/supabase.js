...
const RevGuideDB = {
  ...
  // ============================================
  // Cards (Unified Content)
  // ============================================

  /**
   * Get all cards for current organization
   * @param {Object} filters - Optional filters: cardType, displayMode, objectType, enabled
   */
  async getCards(filters = {}) {
    ...
    if (filters.displayMode) {
      query = query.contains('display_modes', [filters.displayMode]);
    }
    if (filters.objectType) {
      query = query.contains('object_types', [filters.objectType]);
    }
  ...
  /**
   * Create a new card
   * @param {Object} cardData - Card data in snake_case format
   */
  async createCard(cardData) {
    ...
    return client
      .from('cards')
      .insert({ ...cardData, organization_id: orgId })
      .select()
      .single();
  },
   ...
   * @param {string} cardId
   * @param {Object} updates - Fields to update in snake_case format
   */
  ...
  /**
   * Check migration status for current organization
   * @returns {Promise<{data: Object, error}>} Migration status info
   */
  async checkCardsMigrationStatus() {
    const client = await RevGuideAuth.waitForClient();
    const orgId = await this.getOrganizationId();
    if (!orgId) return { data: null, error: new Error('No organization') };

    return client.rpc('check_cards_migration_status', { p_org_id: orgId });
  },

  /**
   * Migrate all legacy content (wiki, banners, plays) to cards
   * @returns {Promise<{data: {wiki, banners, plays, total}, error}>}
   */
  async migrateToCards() {
    const client = await RevGuideAuth.waitForClient();
    const orgId = await this.getOrganizationId();
    if (!orgId) return { data: null, error: new Error('No organization') };

    const { data, error } = await client.rpc('migrate_all_to_cards', { p_org_id: orgId });

    if (!error && data) {
      // Fix relationships after migration
      await client.rpc('fix_card_relationships', { p_org_id: orgId });
    }
  ...
  /**
   * Get cards optimized for content display (filtered by context)
   * @param {string} objectType - Current object type (contacts, companies, deals, tickets)
   * @returns {Promise<{data: Array, error}>}
   */
  async getCardsForDisplay(objectType = null) {
    const client = await RevGuideAuth.waitForClient();
    const orgId = await this.getOrganizationId();
    if (!orgId) return { data: [], error: new Error('No organization') };
    ...
    // If objectType specified, get cards that either:
    // 1. Have empty object_types (show on all)
    // 2. Include this object type in their array
    if (objectType) {
      query = query.or(`object_types.cs.{${objectType}},object_types.eq.{}`);
    }
