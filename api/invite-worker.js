...
export default {
  async fetch(request, env, ctx) {
    ...
    // Route: POST /api/signup-invite-link-oauth - Complete signup for OAuth users
    if (url.pathname === '/api/signup-invite-link-oauth') {
      return handleSignupInviteLinkOAuth(request, env, corsHeaders);
    }
...
// ===========================================
// SIGNUP VIA INVITE LINK (OAUTH)
// ===========================================

async function handleSignupInviteLinkOAuth(request, env, corsHeaders) {
  try {
    const body = await request.json();
    const { authUserId, email, fullName, inviteCode } = body;

    // Validate input
    if (!authUserId) {
      return new Response(JSON.stringify({ error: 'Auth user ID is required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }
    ...
    // 1. Check if user profile already exists for this auth user
    const existingProfileResponse = await fetch(
      `${CONFIG.supabaseUrl}/rest/v1/users?auth_user_id=eq.${authUserId}&select=id,organization_id`,
      {
        headers: {
          'apikey': serviceRoleKey,
          'Authorization': `Bearer ${serviceRoleKey}`,
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Account created successfully',
      user: { id: authUserId, email: email },
      organization: { id: inviteLink.organization_id, name: inviteLink.organization_name }
    }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    ...
    console.error('Signup invite link OAuth error:', error);
    return new Response(JSON.stringify({ error: 'Internal server error' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
