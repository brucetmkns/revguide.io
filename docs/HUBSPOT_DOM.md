...
This document describes the HubSpot CRM DOM structure, injection patterns, and targeting strategies for the RevGuide extension.
...
- [Index/Table Pages](#indextable-pages)
- [Dropdowns & Menus](#dropdowns--menus)
- [Property Extraction](#property-extraction)
- [Injection Strategy](#injection-strategy)
- [Icon Insertion Strategies](#icon-insertion-strategies)
- [CSS for Inline Banners](#css-for-inline-banners)
...
### Index Pages
Index/list pages lazy-load table rows and content as you scroll.
...
### Column Headers
**CRITICAL**: Table headers have deeply nested structures. Target the INNERMOST text container, NOT the `<th>` or `[role="columnheader"]` itself.
...
**Why this matters**: HubSpot table headers wrap text in 5-6 levels of nested divs. If you inject an icon into the `<th>` directly, it appears at the TOP of the cell instead of next to the text.
...
**Targeting**: `[role="option"]`, `[data-option-text="true"]`
...
**Targeting**: `[role="menuitem"]`, `[class*="MenuItem"]`

### Modals & Dialogs
```html
<div role="dialog" class="Modal__...">
  <div class="ModalHeader__...">Title</div>
  <div class="ModalBody__...">...content...</div>
</div>
```
...
HubSpot uses styled-components with randomized class names. Properties cannot be reliably extracted via class names.
...
| `data-menu-item-level` | Nav | `"primary"` or `"secondary"` |
| `data-location` | Nav | `"vertical-nav"` |
| `role="columnheader"` | Index page | Table column headers |
| `role="tablist"` | Record page | Tab navigation |
| `role="menuitem"` | Menus | Menu items |
| `role="option"` | Dropdowns | Select options |
...
| `*ScrollContainer*` | `ScrollContainer__DefaultScrollContainer-...` | Scrollable areas |
...
7. **Table Headers**: Never inject directly into `<th>` elements - text is 6-7 levels deep. Target `[data-test-id*="truncated-object-label"]` instead.

8. **Lazy Loading**: Navigation menus, dropdowns, and list items don't exist until user interaction. Use MutationObserver.
