...
RevGuide supports JSON import/export for transferring content between portals.
...
### Import Modes
- **Replace All**: Deletes existing content in target org before importing
- **Merge / Add**: Adds imported content alongside existing (may create duplicates)

### Security
- DOMPurify sanitizes HTML fields (`definition`, `body`, `message`) to prevent XSS
- IDs are stripped during import; new IDs generated by Supabase

### Key Functions
| Function | File | Purpose |
|----------|------|---------|
| `exportData()` | `admin/pages/settings.js` | Generates JSON backup file |
| `importData()` | `admin/pages/settings.js` | Opens import modal, parses file |
| `confirmImport()` | `admin/pages/settings.js` | Executes import with selected mode |
| `saveStorageData(data, {importMode})` | `admin/shared.js` | Bulk insert to Supabase |
| `sanitizeImportData()` | `admin/shared.js` | XSS protection via DOMPurify |
